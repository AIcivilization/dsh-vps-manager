...
There is also a compact **VPS button in the composer tool row** (next to the model/expert controls). It stays quiet until you need it: most conversations have nothing to do with servers, so the button is just an icon and **fetches nothing on mount** — it only lights up with a machine name once this conversation has actually touched a VPS. It is a launcher, not a second UI: once opened it shows the current machine and its live health, raises at most three suggestions when something actually needs attention, and **every item it offers writes a command into the composer** — you press Enter, the command runs, and the output stays in the conversation. It also has a command box, so the conversation doubles as a terminal for the current machine.
