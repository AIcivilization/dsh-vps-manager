...
There is also a compact **VPS button in the composer tool row** (next to the model/expert controls). A **VPS switch sits in the conversation header**. Turn it on and this conversation is bound to one machine: commands drop the `-h` flag and the AI may omit `host` — you just talk. Turn it off and the conversation has nothing to do with servers again. The binding is per conversation, so another window switching machines cannot affect this one.
