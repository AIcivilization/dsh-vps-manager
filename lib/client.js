...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ComposerEntry(props) {
      ...
      const runQuery = async (recipe) => {
        if (!current) return
        setBusy(recipe.id)
        setError('')
        setResult(null)
        try {
          const res = await api('recipes/query', { id: recipe.id, alias: current.alias })
          setResult({ name: recipe.name, id: recipe.id, text: res.output || res.hint || '（没有输出）' })
        } catch (e) {
          setError(e.message)
        } finally {
          setBusy('')
      ...
      const askAi = async () => {
        const text = [
          `${current.alias}（${current.address || ''}）上「${result.name}」的结果：`,
          '',
          '```',
          result.text.slice(0, 2000),
          '```',
          '',
          '帮我看看有没有问题，需要处理的话说清楚你打算怎么做。',
        ].join('\n')
        if (!handOver(text)) {
          try {
            await navigator.clipboard.writeText(text)
            setError('这个 DSH 版本不支持直接写入对话框，已复制到剪贴板，粘贴即可')
          } catch {
            setError('没能写进对话框，也没能复制')
          }
        }
      }
        ...
        open ? h('div', {
          ...
          current ? h('div', { style: { marginTop: 10 } },
            h('input', {
              ...
              onKeyDown: (e) => {
                if (e.key === 'Enter' && matched[0]) runQuery(matched[0])
              },
            }),
          ...
          // —— 结果 + 交给 AI ——
          result ? h('div', { style: { marginTop: 10 } },
            h('div', { style: S.spread },
              h('div', { style: S.label }, result.name),
              h('div', { style: S.row },
                h(Btn, { kind: 'primary', onClick: askAi }, '交给 AI 看'),
                h(Btn, {
                  onClick: async () => {
                    try {
            ...
            h('div', { style: { ...S.pre, maxHeight: 240 } }, result.text)) : null,

          h('div', { style: { ...S.muted, fontSize: 11, marginTop: 12 } },
            '装软件、系统维护、任务日志、机器设置都在左边栏「VPS 管理」面板；直接跟 AI 说话也能操作这些机器。')) : null)
    }
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, suggestions } }
