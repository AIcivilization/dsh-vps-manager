...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      useEffect(() => {
        ...
        ;(async () => {
          try {
            ...
            // 服务器没记 → 按头部显示的重新绑上；服务器记的是另一台（/vps-use 改过）→ 头部跟服务器走
            if (status.alias !== alias) {
              if (!status.alias) {
                api('session/bind', { sessionId, alias }).catch(() => {})
              } else {
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, alertsFor, readBinding, writeBinding, ballLabel, terminalUrl, normalizeTermPrefs, termChrome, minutesSince } }
