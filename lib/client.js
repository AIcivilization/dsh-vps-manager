  ...
  factory: (require) => {
    ...
    function RecipeRunner({ recipe, hosts, current, onClose }) {
      ...
      const poll = useCallback(async (taskId) => {
        try {
          const res = await api('tasks/status', { alias, taskId })
          setRun((r) => ({ ...r, task: res.task, log: res.log }))
          if (res.task && res.task.state === 'running') {
            pollRef.current = setTimeout(() => poll(taskId), 2000)
          } else {
            setBusy(false)
          }
        } catch (e) {
      ...
      const start = async () => {
        ...
        setRun(null)
        try {
          const res = await api('recipes/run', { id: recipe.id, alias, params })
          setRun({ result: res, log: res.output })
          if (res.status === 'detached' && res.taskId) poll(res.taskId)
          else setBusy(false)
        } catch (e) {
          setError(e.message)
          setBusy(false)
      ...
      return h('div', { style: S.card },
        ...
        h('div', { style: { ...S.row, marginTop: 12 } },
          h(Btn, { kind: 'primary', onClick: start, disabled: busy || !alias }, busy ? '执行中…' : '开始安装'),
          h(Btn, { onClick: detect, disabled: busy }, '重新检测')),

        run ? h('div', { style: { marginTop: 12 } },
          h('div', { style: S.row },
            h(Badge, { tone: run.result?.ok ? 'ok' : run.task?.state === 'running' ? null : 'danger' },
              run.task?.state === 'running' ? '进行中' : run.result?.ok ? '成功' : run.result?.status ?? '结束'),
            h('span', { style: S.muted }, run.result?.hint ?? '')),
          h('div', { style: { ...S.pre, marginTop: 6 } }, run.log || '（暂无输出）')) : null)
    }

    function StoreTab({ overview, onReload }) {
      const [tab, setTab] = useState('builtin')
      const [keyword, setKeyword] = useState('')
      const [openId, setOpenId] = useState('')
      const recipes = (overview?.recipes ?? []).filter((r) => r.kind !== 'query')
      ...
      return h('div', null,
        ...
        list.length === 0
          ? h('div', { style: S.note }, tab === 'builtin' ? '没有匹配的内置菜谱' : '还没有自己的菜谱。让 AI 帮你做成一件事后，它会问你要不要存成菜谱。')
          : null,
        ...
        list.map((r) => h('div', { key: r.id, style: S.card },
          h('div', { style: S.spread },
            ...
            h(Btn, { kind: 'primary', onClick: () => setOpenId(r.id) }, '安装')))))
    }
    ...
    function SettingsSection() {
      ...
      return h('div', { style: S.root },
        ...
        h('div', { style: S.card },
          h('div', { style: S.h2 }, '机器'),
          (d?.hosts ?? []).length === 0 ? h('div', { style: S.muted }, '还没有添加机器，在左边栏「VPS 管理」面板里添加') : null,
          (d?.hosts ?? []).map((host) => h('div', { key: host.alias, style: { ...S.spread, padding: '4px 0' } },
            h('span', null, `${dot(host.reachable)} ${host.alias}　`, h('span', { style: S.muted }, `${host.address || ''} ${host.note || ''}`)),
            h(Btn, { onClick: () => setSelected(host.alias) }, '设置')))),
      ...
      return h('div', { style: S.root },
        ...
        overview.data ? (
          tab === 'machines' ? h(MachinesTab, { overview: overview.data, onReload: overview.reload })
            : tab === 'store' ? h(StoreTab, { overview: overview.data, onReload: overview.reload })
