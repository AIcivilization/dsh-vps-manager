...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *
     * 两个实测教训：
     *  1. InputActions 的公开方法是 setDraft / submit / focus / addAttachments /
     *     pruneAttachments —— **没有 insertText**（那是编辑器内部的）。
     *  2. 命令行要求第 0 字节是 '/'，所以命令不能接在已有文字后面；已有内容时
     *     不擅自覆盖（setDraft 会并进历史，Cmd+Z 未必救得回来），交给用户决定。
     */
    function writeDraft(props, text, { command = true, current = '' } = {}) {
      ...
      if (!command) {
        actions.setDraft(existing.trim() ? `${existing}\n${text}` : text)
        actions.focus?.()
        return { ok: true }
      }
      if (existing.trim() && existing.trim() !== text) {
        return { ok: false, reason: 'draft-busy' }
      }
      ...
      return h('div', { style: { display: 'inline-flex', position: 'relative' } },
        h('button', {
          ...
          style: {
            ...
            alignItems: 'center',
            gap: 6,
            borderColor: on ? '#3b82f6' : undefined,
            color: on ? '#3b82f6' : 'inherit',
            opacity: on ? 1 : 0.75,
          },
          ...
          (hosts ?? []).length === 0
            ...
            : (hosts ?? []).map((host) => h('div', {
                ...
                style: {
                  ...
                  cursor: 'pointer',
                  borderRadius: 6,
                  background: host.alias === alias ? 'rgba(59,130,246,0.14)' : 'transparent',
                },
                onClick: async () => {
                  try {
                ...
                host.alias === alias ? h('span', { style: { color: '#3b82f6' } }, '✓') : null)),
    ...
    function VpsDock(props) {
      ...
      // 读当前草稿：命令不能接在已有文字后面，所以要先知道输入框里有没有东西
      let draft = ''
      try {
        draft = props?.useInput ? (props.useInput((st) => st?.draft ?? '') ?? '') : ''
      } catch {
        draft = ''
      }
      ...
      useEffect(() => {
        ...
        ;(async () => {
          try {
            const [status, health, overview, tasks] = await Promise.all([
            ...
            setInfo({
              status,
              health: health.output || health.hint || '',
              running: (tasks.tasks ?? []).filter((t) => t.state === 'running'),
            })
            setQueries((overview.recipes ?? []).filter((r) => r.kind === 'query' && r.id !== 'probe'))
          } catch (e) {
      ...
      if (!alias) return null // 没打开开关的对话：零渲染、零请求

      const facts = info?.status?.facts ?? {}
      const matched = keyword
        ? queries.filter((r) => `${r.id}${r.name}${r.desc}`.toLowerCase().includes(keyword.toLowerCase()))
        : queries.slice(0, 6)
      const push = (text, opts = {}) => {
        setError('')
        const res = writeDraft(props, text, { current: draft, ...opts })
        if (res.ok) {
          setPending('')
          return
        }
        if (res.reason === 'draft-busy') setPending(text)
        else setError(`这个 DSH 版本没把输入框动作传给插件，请手动输入：${text}`)
      }

      const force = (text) => {
        props?.inputActions?.setDraft?.(text)
        props?.inputActions?.focus?.()
        setPending('')
      }
      ...
      return h('div', {
        ...
        h('div', { style: S.spread },
          h('div', { style: S.row },
            h('span', { style: { fontWeight: 600, color: '#3b82f6' } }, `${dot(info?.status?.reachable)} ${alias}`),
            h('span', { style: S.muted }, info?.status?.address ?? ''),
            info?.health
              ? h('span', { style: S.muted }, info.health.split('\n')[0].slice(0, 70))
              : h('span', { style: S.muted }, '读取中…')),
          (info?.running ?? []).length
            ? h('span', { style: S.badge('ok') }, `${info.running.length} 个任务在跑`)
            : null),

        h(ErrorBar, { error }),

        suggestions({ reachable: info?.status?.reachable }, facts).map((sg, i) =>
          h('div', { key: i, style: { color: '#e5a13b', marginTop: 4 } },
            sg.kind === 'query'
              ? h('a', {
                  href: '#',
        ...
        h('div', { style: { ...S.row, marginTop: 6 } },
          matched.map((r) => h(Btn, {
            key: r.id,
            onClick: () => push(commandForQuery(r.id)),
            title: `${r.desc}（填进输入框，回车执行）`,
          }, r.name)),
          h('input', {
            style: { ...S.input, width: 120, fontSize: 12 },
            value: keyword,
            placeholder: '找查询…',
        ...
        h('div', { style: { ...S.row, marginTop: 6 } },
          h('input', {
            ...
            value: shell,
            placeholder: `在 ${alias} 上敲命令（回车填进输入框）`,
            onChange: (e) => setShell(e.target.value),
            onKeyDown: (e) => {
              if (e.key === 'Enter' && shell.trim()) {
                push(`/vps-sh ${shell.trim()}`)
                setShell('')
              }
            },
          }),
        ...
        pending
          ? h('div', { style: { ...S.note, marginTop: 6 } },
              '输入框里已经有内容，没有覆盖它。',
              h('a', {
                href: '#',
                style: { marginLeft: 6, color: '#3b82f6' },
                onClick: (e) => {
                  e.preventDefault()
                  force(pending)
                },
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, suggestions, commandForQuery, readBinding, writeBinding, writeDraft } }
