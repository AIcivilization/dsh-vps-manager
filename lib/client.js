...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return h('div', { style: S.root },
        ...
        h('div', { style: S.card },
          ...
          h('div', { style: { ...S.muted, fontSize: 12, lineHeight: 1.9 } },
            ...
            h('div', null, '⑤ 想跟在 ssh 里一样自己敲、用菜单脚本：点对话头部方块后面的 ', h('span', { style: S.mono }, '>_'), '，输入框下方就是终端'),
    ...
    function VpsToggle(props) {
      ...
      // 绑定了机器才出现：点开 / 收起输入框下方的终端
      const terminalButton = alias ? h('button', {
        key: 'terminal',
        type: 'button',
        onClick: () => terminal.setOpen(!terminal.open),
        title: terminal.open ? '收起终端（会断开连接）' : `打开 ${alias} 的终端：跟在 ssh 里一样操作`,
        style: {
          height: 17,
          padding: '0 4px',
          marginLeft: 2,
