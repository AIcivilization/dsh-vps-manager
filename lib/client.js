...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 一切动作都落成对话里的命令：写进输入框，由你按回车执行 */
    function pushCommand(props, text) {
    ...
    function VpsDock(props) {
      ...
      const matched = keyword
        ...
        : queries.slice(0, 6)
      const push = (text) => {
        if (!pushCommand(props, text)) setError(`请手动输入：${text}`)
      }
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, suggestions, commandForQuery, readBinding, writeBinding } }
