...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VpsTerminal({ sessionId, alias, onClose }) {
      ...
      useEffect(() => {
        ...
        loadXterm().then(({ Terminal, FitAddon }) => {
          ...
          term = new Terminal({
            ...
            scrollback: 5000,
            theme: { background: TERM_BG, foreground: '#d4d4d4', cursor: '#d4d4d4', selectionBackground: '#264f78' },
          })
      ...
      return h('div', {
        style: { marginTop: 6, border: line, borderRadius: 8, overflow: 'hidden', background: TERM_BG },
      },
        h('div', {
          style: {
            display: 'flex',
          ...
          h('span', { style: { fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontWeight: 600 } }, alias),
          h('span', { style: { color: status === 'open' ? '#3fb950' : status === 'closed' ? '#f85149' : '#999' } }, statusText),
          h('span', { style: { flex: 1 } }),
          status === 'closed' ? barBtn('重新连接', () => setAttempt((n) => n + 1), '开一个新的 shell') : null,
          barBtn('收起', onClose, '收起终端（会断开连接，服务器上这个 shell 随之结束）')),
        note ? h('div', { style: { padding: '4px 10px', fontSize: 12, color: '#f0b72f', background: '#2d2a1f' } }, note) : null,
        h('div', {
          ref: boxRef,
          style: {
