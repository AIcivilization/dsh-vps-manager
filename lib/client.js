...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // —— 对话头部的开关：VPS + 一串小球，不弹任何东西 ——
    //
    // 「VPS」后面跟机器数量个小球：绿 = 这个对话正在操作它，红 = 没有。
    // 一台时球里不写数字，多台时写 1234。点哪个绑哪个，再点一次关掉；
    ...
    function VpsToggle(props) {
      ...
      // 小球：绿 = 这个对话正在操作它，红 = 没有。每次只能亮一个。
      const ball = (key, text, on, title, onClick) => h('button', {
        key,
        type: 'button',
        onClick,
        title,
      ...
      if (!hosts.length) {
        return wrap(ball('load', busy ? '·' : '', false, error || '点一下绑定这个对话要操作的 VPS', () => click(null)))
      }
