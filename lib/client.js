...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ComposerEntry() {
      ...
      const runQuick = async (id, name) => {
        if (!current) return
        setBusy(id)
        setError('')
        setResult(null)
        try {
          const res = await api('recipes/query', { id, alias: current.alias })
          setResult({ name, text: res.output || res.hint || '（没有输出）' })
        } catch (e) {
          setError(e.message)
        } finally {
          setBusy('')
      ...
      return h('div', { style: { position: 'relative', display: 'inline-flex' } },
        ...
        open ? h('div', {
          style: {
            ...
            borderRadius: 10,
            padding: 12,
            background: 'var(--dsh-bg, rgba(32,32,36,0.98))',
            boxShadow: '0 8px 30px rgba(0,0,0,0.35)',
            fontSize: 13,
          },
        },
          h('div', { style: S.spread },
            h('div', { style: { fontWeight: 600 } }, 'VPS'),
          ...
          hosts.length === 0
            ...
            : h('div', { style: { marginTop: 8 } },
                hosts.map((host) => h('div', {
                  key: host.alias,
                  style: { ...S.spread, padding: '4px 0', cursor: 'pointer' },
                  onClick: async () => {
                    try {
                      await api('host/current', { alias: host.alias })
                      data.reload()
                  ...
                  h('span', null, `${dot(host.reachable)} ${host.alias}`,
                    host.alias === data.data?.current ? h('span', { style: S.muted }, ' ★ 当前') : null),
                  h('span', { style: { ...S.muted, ...S.mono } }, host.address || '')))),

          current ? h('div', { style: { marginTop: 10 } },
            h('div', { style: S.label }, `在 ${current.alias} 上快速看一眼（只读，不花 token）`),
            h('div', { style: S.row },
              QUICK.map(([id, name]) => h(Btn, {
                key: id,
                onClick: () => runQuick(id, name),
                disabled: Boolean(busy),
              }, busy === id ? '…' : name)))) : null,

          result ? h('div', { style: { marginTop: 10 } },
            h('div', { style: S.label }, result.name),
            h('div', { style: { ...S.pre, maxHeight: 220 } }, result.text)) : null,

          h('div', { style: { ...S.muted, fontSize: 11, marginTop: 12 } },
            '装软件、系统维护、任务日志、机器设置在左边栏「VPS 管理」面板里；',
            '直接跟 AI 说话也能操作这些机器。')) : null)
    }
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api } }
