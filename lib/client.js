...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // —— 对话头部的开关：就是开关本身，不弹任何东西 ——
    //
    // 一台机器时按钮是「VPS on / VPS off」；多台时挂编号后缀「VPS1 on」「VPS2 off」，
    // 点哪个就把这个对话绑到哪台，再点一次关掉。绑定是单选：开 VPS2 会自动关掉 VPS1。
    //
    ...
    /** 按钮文字：一台不带编号，多台带编号 */
    function toggleLabel(index, total, on) {
      return `${total > 1 ? `VPS${index + 1}` : 'VPS'} ${on ? 'on' : 'off'}`
    }
    ...
    function VpsToggle(props) {
      ...
      const click = async (target) => {
        ...
        try {
          ...
          if (!target) {
            ...
            else if (list.length === 0) setError('还没有添加机器：左边栏「VPS 管理」→「+ 添加机器」')
            return
          }
      ...
      const style = (on) => ({
        ...
        alignItems: 'center',
        gap: 5,
        border: `1px solid ${on ? T.ok : T.danger}`,
        color: on ? T.ok : 'inherit',
        opacity: on ? 1 : 0.6,
      })
      ...
      if (!hosts.length) {
        return h('button', {
          type: 'button',
          onClick: () => click(null),
          title: error || '点一下把这个对话绑定到你的 VPS',
          style: style(false),
        }, h(PanelIcon, { size: 13 }), busy ? '…' : 'VPS off')
      }

      return h('span', { style: { display: 'inline-flex', gap: 4 } },
        hosts.map((host, i) => {
          ...
          return h('button', {
            ...
            title: on
              ...
              : `点一下让这个对话操作 ${host.alias}${host.note ? `（${host.note}）` : ''}`,
            style: style(on),
          }, h(PanelIcon, { size: 13 }), busy === host.alias ? '…' : toggleLabel(i, hosts.length, on))
        }))
    }
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, alertsFor, readBinding, writeBinding, toggleLabel } }
