...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const line = '1px solid rgba(127,127,127,0.25)'
    const S = {
      ...
      btn: (kind, disabled) => ({
        border: kind === 'primary' ? '1px solid #3b82f6' : line,
        background: kind === 'primary' ? '#3b82f6' : 'transparent',
        color: kind === 'primary' ? '#fff' : kind === 'danger' ? '#e5534b' : 'inherit',
        borderRadius: 6,
        padding: '5px 12px',
      ...
      badge: (tone) => ({
        ...
        borderRadius: 10,
        border: line,
        color: tone === 'danger' ? '#e5534b' : tone === 'ok' ? '#2ea043' : 'inherit',
        opacity: tone ? 1 : 0.7,
        whiteSpace: 'nowrap',
      }),
      err: { border: '1px solid #e5534b', color: '#e5534b', borderRadius: 6, padding: '8px 10px', marginBottom: 10 },
      note: { border: line, borderRadius: 6, padding: '8px 10px', marginBottom: 10, background: 'rgba(127,127,127,0.08)' },
    ...
    function MachineSettings({ alias, onBack, onChanged }) {
      ...
      return h('div', null,
        ...
        h('div', { style: { ...S.card, borderColor: 'rgba(229,83,75,0.5)' } },
          h('div', { style: { ...S.h2, color: '#e5534b' } }, '危险区'),
          !removing
    ...
    /** 根据体检事实给出「现在值得做的事」，最多三条 */
    function suggestions(host, facts) {
      const out = []
      if (host && host.reachable === false) {
        out.push({ text: '这台机器连不上，去机器设置页看看', kind: 'panel' })
        return out
      }
      const pct = Number(String(facts.disk_pct ?? '').replace('%', ''))
      if (pct >= 85) out.push({ text: `磁盘已用 ${facts.disk_pct}，看看谁占的`, kind: 'query', id: 'disk' })
      const mem = Number(facts.mem_total_mb || 0)
      if (mem > 0 && Number(facts.swap_total_mb || 0) === 0 && mem <= 2048) {
        out.push({ text: '内存不大又没有 swap，建议加虚拟内存', kind: 'panel' })
      }
      if (facts.fail2ban === '0') out.push({ text: '没装 fail2ban，先看看有没有人在爆破', kind: 'query', id: 'login-history' })
      if (facts.auto_updates === '0') out.push({ text: '没开自动安全更新，看看有多少待更新', kind: 'query', id: 'updates' })
      return out.slice(0, 3)
    }

    // 查询菜谱 → 对应的斜杠命令（没有专属命令的走通用的 /vps-q）
    const COMMAND_FOR = {
      sysinfo: '/vps-sysinfo', disk: '/vps-disk', ports: '/vps-ports', services: '/vps-services',
      net: '/vps-net', 'docker-ps': '/vps-docker', health: '/vps-ping', logs: '/vps-logs ',
    }
    const commandForQuery = (id) => COMMAND_FOR[id] ?? `/vps-q ${id}`

    /** 这个对话有没有真的在操作 VPS（用过才点亮，按会话记住） */
    function readActive(sessionId) {
      if (!sessionId) return false
      try {
        return window.localStorage?.getItem(`dsh-vps:active:${sessionId}`) === '1'
      } catch {
        return false
      }
    }
    ...
    function markActive(sessionId) {
      if (!sessionId) return
      try {
        window.localStorage?.setItem(`dsh-vps:active:${sessionId}`, '1')
      } catch {
        // 隐私模式下写不了，只影响按钮显不显示机器名
      }
    }
    ...
    function ComposerEntry(props) {
      ...
      const [active, setActive] = useState(() => readActive(sessionId))
      // 不操作 VPS 的对话占大多数，所以**挂载时什么都不请求**，点开才加载
      const [data, setData] = useState({ loading: false, data: null, error: '' })
      ...
      const load = useCallback(async () => {
        ...
        try {
          setData({ loading: false, data: await api('overview', {}), error: '' })
        } catch (e) {
          setData({ loading: false, data: null, error: e.message })
        }
      }, [])

      const hosts = data.data?.hosts ?? []
      const current = hosts.find((x) => x.alias === data.data?.current) ?? hosts[0]
      // 平时只是一个安静的图标；这个对话用过 VPS 之后才显示是哪台机器
      const label = active && current ? `${dot(current.reachable)} ${current.alias}` : 'VPS'
      const queries = (data.data?.recipes ?? []).filter((r) => r.kind === 'query' && r.id !== 'probe')
      const matched = keyword
        ? queries.filter((r) => `${r.id}${r.name}${r.desc}`.toLowerCase().includes(keyword.toLowerCase()))
        : queries

      // 打开时：拉一次健康状态和任务，不用用户先点
      const refresh = useCallback(async (alias) => {
        if (!alias) return
        setHealth({ loading: true })
        try {
          const [h, t] = await Promise.all([
            api('recipes/query', { id: 'health', alias }),
            api('tasks/list', { alias }).catch(() => ({ tasks: [] })),
          ])
          setHealth({ text: h.output || h.hint || '' })
          setTasks((t.tasks ?? []).filter((x) => x.state === 'running'))
        } catch (e) {
      ...
      const toggle = () => {
        if (!open) {
          if (btnRef.current) {
            ...
            setAnchor({ left: r.left, bottom: window.innerHeight - r.top + 8 })
          }
          setKeyword('')
          load().then(() => refresh(current?.alias))
      ...
      /** 所有动作统一走这里：把命令推进对话框，由你按回车执行，结果留在对话里 */
      const pushToComposer = (text, { keepOpen = false } = {}) => {
        const actions = props?.inputActions
        if (actions && typeof actions.insertText === 'function') {
          actions.insertText(text)
          if (typeof actions.focus === 'function') actions.focus()
          markActive(sessionId)
          setActive(true)
          if (!keepOpen) setOpen(false)
          return true
        }
        setError('这个 DSH 版本没把输入框动作传给插件，请手动输入：' + text)
        return false
      }

      const pushQuery = (recipe) => pushToComposer(commandForQuery(recipe.id))

      const askAi = () => {
        const text = [
          `${current.alias}（${current.address || ''}）现在的状态：`,
          '',
          '```',
          (health?.text ?? '').slice(0, 1200),
          '```',
          '',
          '帮我看看有没有要处理的地方；要动手的话先说清楚你打算怎么做。',
        ].join('\n')
        pushToComposer(text)
      }

      const row = { display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0' }
      ...
      return h('div', { style: { position: 'relative', display: 'inline-flex' } },
        h('button', {
          ...
          type: 'button',
          onClick: toggle,
          title: 'VPS：状态、查询、交给 AI',
          style: { ...S.btn(null, false), padding: '3px 10px', fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 6 },
        }, h(PanelIcon, { size: 14 }), label),
        ...
        open ? h('div', {
          ...
          style: {
            ...
            borderRadius: 10,
            padding: 12,
            background: 'var(--dsh-bg, rgba(30,30,34,0.985))',
          ...
          h('div', { style: S.spread },
            hosts.length > 1
              ? h('div', { style: { minWidth: 170 } },
                  h(Select, {
                    value: current?.alias ?? '',
                    onChange: async (alias) => {
                      try {
                        await api('host/current', { alias })
                        await load()
                        markActive(sessionId)
                        setActive(true)
                        refresh(alias)
                    ...
                    options: hosts.map((x) => ({ value: x.alias, label: `${dot(x.reachable)} ${x.alias}${x.note ? ` · ${x.note}` : ''}` })),
                  }))
              : h('div', { style: { fontWeight: 600 } }, current ? `${dot(current.reachable)} ${current.alias}` : 'VPS'),
            h('div', { style: S.row },
              h(Btn, { onClick: () => { load(); refresh(current?.alias) } }, '刷新'),
              h(Btn, { onClick: () => setOpen(false) }, '关闭'))),
          current ? h('div', { style: { ...S.muted, ...S.mono, marginTop: 2 } }, `${current.address || ''}${current.note ? ` · ${current.note}` : ''}`) : null,

          h(ErrorBar, { error: error || data.error }),

          data.loading && !data.data
            ? h('div', { style: { ...S.muted, marginTop: 8 } }, '读取中…')
            : hosts.length === 0
            ? h('div', { style: { ...S.note, marginTop: 8 } }, '还没有机器。左边栏「VPS 管理」→「+ 添加机器」')
            : h('div', { style: { ...S.pre, marginTop: 8, maxHeight: 120 } },
                health?.loading ? '读取中…' : health?.error ? `读不到状态：${health.error}` : (health?.text || '（没有输出）')),

          // —— 有问题才出现的建议 ——
          current ? suggestions(current, data.data?.hosts?.find((x) => x.alias === current.alias)?.facts ?? current.facts ?? {}).map((sg, i) =>
            h('div', { key: i, style: { ...row, color: '#e5a13b' } },
              h('span', null, '•'),
              sg.kind === 'query'
                ? h('a', {
                    href: '#',
                    ...
                    onClick: (e) => {
                      e.preventDefault()
                      const r = queries.find((q) => q.id === sg.id)
                      if (r) pushQuery(r)
                    },
                  }, sg.text)
                : h('span', null, sg.text))) : null,

          // —— 有任务才出现 ——
          (tasks ?? []).length
            ? h('div', { style: { ...S.note, marginTop: 8 } },
                `${tasks.length} 个任务正在这台机器上跑：`,
                tasks.slice(0, 3).map((t) => h('div', { key: t.taskId, style: { ...S.mono, fontSize: 11 } },
                  `${t.meta?.recipeId ?? t.meta?.action ?? ''} ${t.taskId}`)),
                h('div', { style: { ...S.muted, fontSize: 11, marginTop: 4 } }, '进度和日志在面板的「任务」页'))
            : null,

          // —— 可搜索的查询列表（全部，不是几个固定按钮）——
          current ? h('div', { style: { marginTop: 10 } },
            h('input', {
              ref: searchRef,
              style: { ...S.input, width: '100%' },
              value: keyword,
              placeholder: `搜索查询（共 ${queries.length} 条，回车执行第一个）`,
              onChange: (e) => setKeyword(e.target.value),
              onKeyDown: (e) => {
                if (e.key === 'Enter' && matched[0]) pushQuery(matched[0])
              },
            }),
            h('div', { style: { ...S.row, marginTop: 6 } },
              matched.slice(0, 8).map((r) => h(Btn, {
                key: r.id,
                onClick: () => pushQuery(r),
                title: `${r.desc}（会把命令填进对话框）`,
              }, r.name))),
            matched.length === 0 ? h('div', { style: S.muted }, '没有匹配的查询') : null) : null,

          // —— 像 SSH 一样直接敲命令：填成 /vps-sh <命令> 推进对话框 ——
          current ? h('div', { style: { marginTop: 10 } },
            h('input', {
              style: { ...S.input, width: '100%' },
              value: shell,
              placeholder: `在 ${current.alias} 上直接敲命令（回车填进对话框）`,
              onChange: (e) => setShell(e.target.value),
              onKeyDown: (e) => {
                if (e.key === 'Enter' && shell.trim()) {
                  pushToComposer(`/vps-sh ${shell.trim()}`)
                  setShell('')
                }
              },
            }),
            h('div', { style: { ...S.muted, fontSize: 11, marginTop: 4 } },
              '命令在当前机器上执行，输出直接进对话，不花 token；高危命令会先拦下来让你加 --yes')) : null,

          // —— 把当前状态交给 AI ——
          health?.text ? h('div', { style: { ...S.row, marginTop: 10 } },
            h(Btn, { kind: 'primary', onClick: askAi }, '把这台机器的状态交给 AI'),
            h(Btn, { onClick: () => pushToComposer(`看看 ${current.alias} 的`, { keepOpen: true }) }, '在对话里提这台机器')) : null,

          h('div', { style: { ...S.muted, fontSize: 11, marginTop: 12 } },
            '点任何一项都是把命令填进对话框，你按回车才执行，结果留在对话里。',
            '装软件和系统维护在左边栏「VPS 管理」面板。')) : null)
    }
    ...
    function apply(ctx) {
      ...
      try {
        // 输入框工具行（「专家」那一排）：会话内随手可点
        ctx.slots.inject('conversation.input.left', () =>
          ctx.slots.register({ name: 'conversation.input.left', id: 'vps-manager', order: 40 }, ComposerEntry))
      } catch (error) {
        console.warn('[dsh-vps-manager] 输入框入口注册失败', error)
      }
      try {
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, suggestions, commandForQuery } }
