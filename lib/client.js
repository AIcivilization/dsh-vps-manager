...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return h('div', { style: S.root },
          ...
          h('div', { style: { marginTop: 10 } },
            h('label', { style: S.row },
              h('input', {
                ...
                onChange: (e) => setSettings({ ...settings, allowTerminalRemote: e.target.checked }),
              }),
              h('span', null, '允许从其他设备打开 VPS 终端')),
            h('div', { style: { ...S.muted, fontSize: 11, marginTop: 2, marginLeft: 24 } },
        ...
        h('div', { style: S.card },
          ...
          h('div', { style: { ...S.muted, fontSize: 12, lineHeight: 1.9 } },
            h('div', null, '① 在对话头部点「VPS」后面的编号方块，这个对话就绑到那台机器（编号和上面列表一致，点另一个编号切换）'),
            ...
            h('div', null, '⑤ 想跟在 ssh 里一样自己敲、用菜单脚本：点对话头部「VPS」后面的 ', h('span', { style: S.mono }, '>_'), '，输入框下方就是终端'),
    ...
    function VpsToggle(props) {
      ...
      const terminal = useTerminalOpen(sessionId)
      ...
      const terminalButton = h('button', {
        ...
        title: !alias
          ...
          : terminal.open ? '收起终端（会断开连接）' : `打开 ${alias} 的终端：跟在 ssh 里一样操作`,
        style: {
          height: 17,
          padding: '0 4px',
          marginRight: 2,
          borderRadius: 4,
          border: terminal.open ? `1px solid ${T.accent}` : line,
          background: terminal.open ? T.accent : 'transparent',
          color: terminal.open ? '#fff' : 'inherit',
    ...
    //
    // xterm.js 有 300 多 KB，只在第一次点开终端时才从插件自己的路由加载。
    // 收起终端、切走对话、刷新页面都会断开，服务器上那个 shell 随之结束。
    ...
    const terminalOpenState = new Map() // sessionId → true

    /** 终端开没开：头部按钮和输入框下方是两个组件，用事件保持同步 */
    function useTerminalOpen(sessionId) {
      const [open, setOpenState] = useState(() => terminalOpenState.get(sessionId) === true)
      useEffect(() => {
        setOpenState(terminalOpenState.get(sessionId) === true)
        const onChange = (e) => {
          if (e?.detail?.sessionId === sessionId) setOpenState(e.detail.open === true)
        }
        window.addEventListener(TERM_EVENT, onChange)
        return () => window.removeEventListener(TERM_EVENT, onChange)
      }, [sessionId])
      const setOpen = useCallback((next) => {
        if (next) terminalOpenState.set(sessionId, true)
        else terminalOpenState.delete(sessionId)
        setOpenState(Boolean(next))
        try {
          window.dispatchEvent(new CustomEvent(TERM_EVENT, { detail: { sessionId, open: Boolean(next) } }))
        } catch {
    ...
    function terminalUrl(origin, sessionId, cols, rows) {
      const url = new URL('/api-vps/ws/terminal', origin)
      url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
      url.search = new URLSearchParams({ sessionId, cols: String(cols), rows: String(rows) }).toString()
      return url.toString()
    }
    ...
    function loadXterm() {
      ...
      try {
        if (!document.querySelector('link[data-dsh-vps-xterm]')) {
      ...
      xtermLoading = Promise.all([import(asset('xterm.mjs')), import(asset('addon-fit.mjs'))])
    ...
    /** 按 DSH 当前主题拼 xterm 的配色 */
    function terminalTheme(anchor) {
      const dark = isDarkTheme()
      const color = (v) => resolveCss(anchor, 'color', v)
      const background = color(DS.bg)
      const accent = color(DS.accent)
      return {
        ...
        scrollbarSliderBackground: color(DS.scrollbar),
        scrollbarSliderHoverBackground: withAlpha(color(DS.tertiary), 0.5),
        scrollbarSliderActiveBackground: withAlpha(color(DS.tertiary), 0.7),
      }
    }

    function VpsTerminal({ sessionId, alias, onClose }) {
      const boxRef = useRef(null)
      const [status, setStatus] = useState('loading') // loading | connecting | open | closed
      ...
      useEffect(() => {
        ...
        let fitTimer = null
        // 终端里按的键不能冒泡给 DSH：Esc、快捷键、粘贴图片都会被输入框那边接走
        ...
        loadXterm().then(({ Terminal, FitAddon }) => {
          ...
          if (!token) {
            setStatus('closed')
            setNote('页面里没有 VPS 管理的令牌：刷新页面再试')
            return
          }
          term = new Terminal({
            cursorBlink: true,
            fontSize: 13,
            fontFamily: resolveCss(box, 'fontFamily', DS.code) || 'monospace',
            scrollback: 5000,
            theme: terminalTheme(box),
          })
          // DSH 切换深浅色：改的是 body 上的属性和变量，盯着它换配色
          themeObserver = new MutationObserver(() => {
            try {
              term.options.theme = terminalTheme(box)
            } catch {
              // 终端已经销毁
            }
          })
          ...
          try {
            ...
            // 容器还没尺寸，ResizeObserver 之后会再调
          }

          setStatus('connecting')
          let opened = false
          socket = new WebSocket(terminalUrl(window.location.origin, sessionId, term.cols, term.rows), [TERMINAL_PROTOCOL, token])
          ...
          socket.onmessage = (event) => {
            ...
            if (msg?.t === 'error') setNote(msg.message || '终端出错了')
            else if (msg?.t === 'exit') {
              setNote(msg.hint ? `连接断开：${msg.hint}` : msg.code === 0 ? '已退出' : `连接断开（退出码 ${msg.code ?? msg.signal}）`)
            }
          }
          socket.onclose = () => {
            if (disposed) return
            setStatus('closed')
            if (!opened) setNote((n) => n || '连不上终端服务：可能是登录过期或 DSH 刚重启过，刷新页面再试')
            try {
              term.write('\r\n\x1b[2m[连接已断开]\x1b[0m\r\n')
            } catch {
              // 终端已经销毁
          ...
          term.onData((data) => {
            if (socket?.readyState === 1) socket.send(JSON.stringify({ t: 'i', d: data }))
          })
          term.onResize(({ cols, rows }) => {
            if (socket?.readyState === 1) socket.send(JSON.stringify({ t: 'r', cols, rows }))
          })
          ...
          if (disposed) return
          setStatus('closed')
          setNote(`终端组件加载失败：${error?.message ?? error}`)
        })
      ...
      }, [sessionId, alias, attempt])

      const statusText = { loading: '加载中…', connecting: '连接中…', open: '已连接', closed: '已断开' }[status]
      const barBtn = (label, onClick, title) => h('button', {
        type: 'button',
        onClick,
        title,
      ...
      const dotColor = status === 'open' ? DS.ok : status === 'closed' ? DS.danger : DS.tertiary
      ...
      return h('div', {
        ...
        h('div', {
          style: {
            ...
            borderBottom: `0.5px solid ${DS.divider}`,
            fontSize: 13,
          },
        },
          h('span', { style: { width: 6, height: 6, borderRadius: '50%', background: dotColor, flex: '0 0 auto' } }),
          h('span', { style: { fontWeight: 500, color: DS.text } }, alias),
          h('span', { style: { color: DS.tertiary, fontSize: 12 } }, statusText),
          h('span', { style: { flex: 1 } }),
          status === 'closed' ? barBtn('重新连接', () => setAttempt((n) => n + 1), '开一个新的 shell') : null,
          barBtn('收起', onClose, '收起终端（会断开连接，服务器上这个 shell 随之结束）')),
        note ? h('div', { style: { padding: '6px 14px', fontSize: 12, color: DS.secondary, background: DS.tip } }, note) : null,
        h('div', {
          ref: boxRef,
          style: {
            height: readTerminalHeight(),
            minHeight: 120,
            maxHeight: '80vh',
            resize: 'vertical', // 右下角拖高度；宽度跟着输入框走
            overflow: 'hidden',
    ...
    /** 输入框下方：终端（点开时）+ 状态提醒（有事时） */
    function VpsDock(props) {
      const sessionId = props?.sessionId ? String(props.sessionId) : ''
      const { alias } = useBinding(sessionId)
      const terminal = useTerminalOpen(sessionId)
      const alert = h(VpsAlert, { key: 'alert', ...props })
      if (!alias || !terminal.open) return alert
      return h(React.Fragment, null,
        h(VpsTerminal, { key: `terminal:${sessionId}:${alias}`, sessionId, alias, onClose: () => terminal.setOpen(false) }),
        alert)
    }
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, alertsFor, readBinding, writeBinding, ballLabel, terminalUrl } }
