...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function aliasProblem(alias) {
      ...
      if (/[^\x00-\x7f]/.test(a)) return `别名只能用英文字母、数字和 - _ .（它会写进 SSH 配置）。「${a}」这样的中文名字写到备注里，别名留空会自动起一个`
      return '别名只能用英文字母、数字和 - _ .，而且要以字母或数字开头'
    }
        ...
        h('div', { style: grid2 },
          h('div', null,
            ...
            aliasProblem(form.alias)
              ...
              : h('div', { style: hint }, '英文字母、数字和 - _ .，比如 la、hk-web；中文名字写备注')),
          h('div', null,
