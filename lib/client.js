  ...
  factory: (require) => {
    ...
    function VpsAlert(props) {
      ...
      useEffect(() => {
        ...
        ;(async () => {
          ...
          // 头部显示的绑定和服务器记的不一致（实测：头部是绿的，/vps-disk 却说没开开关）：
          // 服务器没记 → 按头部显示的重新绑上再测；服务器记的是另一台（/vps-use 改过）→ 头部跟服务器走
          if (status && status.alias !== alias) {
            if (!status.alias) {
              await api('session/bind', { sessionId, alias }).catch(() => {})
              await checkNow(sessionId, alias, true)
            } else {
    ...
    function injectStyles() {
      try {
        ...
        style.textContent = '@keyframes dshVpsPulse{0%,100%{opacity:1}50%{opacity:.4}}'
    ...
    // 给测试用的内部句柄（浏览器里没人碰它）
    module.exports = { name, inject, apply, __test: { api, alertsFor, readBinding, writeBinding, ballLabel, chipTone, terminalUrl, normalizeTermPrefs, termChrome, minutesSince } }
