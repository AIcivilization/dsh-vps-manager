...
export async function requestApproval(ctx, req) {
  ...
  try {
    ...
    if (outcome === APPROVAL_OUTCOMES.rejected) return { decision: DECISION.deny, hint: '用户拒绝了这次操作' }
    if (outcome === APPROVAL_OUTCOMES.cancelled) return { decision: DECISION.deny, hint: '确认被取消了（对话被中断），没有执行' }
