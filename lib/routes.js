// lib/routes.js — 面板路由 /api-vps/*（设计第三节「面板 ↔ host 通信」）
//
// 面板没有 Session 绑定，调 host 只能走 HTTP。而 dsh-host-webserver 本身没有任何
...
//      默认关闭（面板降级为只读），设置里可以手动打开
//
// 面板里的「开始安装」按钮本身就是用户的同意，所以走 preApproved（面板弹不出审批框：
// 审批要求处于未结束的轮次中）。

import { randomBytes } from 'node:crypto'
import { unlink } from 'node:fs/promises'
import { join } from 'node:path'
import { hostsAction, probeHost, recipeAction, taskAction } from './actions.js'
...
import { saveUserRecipe } from './recipe-store.js'
...
export function registerRoutes(ctx, deps = {}) {
  ...
  route('host/current', async ({ alias }) => {
    const doc = await readHosts(env)
    if (alias && !doc.hosts[alias]) throw new Error(`没有登记过这台机器：${alias}`)
  ...
  // —— 菜谱 ——
  route('recipes/show', async ({ id, alias }) =>
    recipeAction({ ctx, action: 'show', id, alias, env, runner, source: 'panel' }))

  // 面板默认只等几秒就转后台，之后靠任务页轮询，避免长安装把 HTTP 请求挂住
  route('recipes/run', async ({ id, alias, params, force, waitSeconds = 8 }) =>
    recipeAction({
      ...
      runner,
      source: 'panel',
      preApproved: true, // 面板上的「开始安装」就是同意
    }), { write: true })

  // 快捷查询：只允许 kind=query 的菜谱，因此不受局域网只读降级影响
  route('recipes/query', async ({ id, alias }) => {
    const { byId } = await loadRecipes({ env })
    const recipe = byId.get(id)
    if (!recipe) throw new Error(`没有这条菜谱：${id}`)
    if (recipe.kind !== 'query') throw new Error(`${id} 不是查询类菜谱，请走应用商店或系统维护页`)
    return recipeAction({ ctx, action: 'run', id, alias, env, runner, source: 'panel', preApproved: true })
  })

  // 面板异步执行完任务后，单独跑一次 verify（只读）
  route('recipes/verify', async ({ id, alias }) =>
    recipeAction({ ctx, action: 'verify', id, alias, env, runner, source: 'panel' }))

  route('recipes/save', async ({ recipe }) =>
    saveUserRecipe({ ctx, recipe, env, source: 'panel', preApproved: true }), { write: true })

  route('recipes/delete', async ({ id }) => {
    const { byId } = await loadRecipes({ env })
    const recipe = byId.get(id)
    if (!recipe) throw new Error(`没有这条菜谱：${id}`)
    if (recipe.source === 'builtin') throw new Error('内置菜谱不能删除')
    await unlink(recipe.file)
    return { id, file: recipe.file }
  }, { write: true })
  ...
  route('tasks/cancel', async ({ alias, taskId }) =>
    taskAction({ ctx, alias, action: 'cancel', taskId, env, runner, source: 'panel', preApproved: true }), { write: true })
  ...
  route('session/status', async ({ sessionId }) => {
    ...
    return { alias, address: st.address ?? '', reachable: st.reachable ?? null, facts: st.facts ?? {} }
  })
  ...
  route('audit/list', async ({ limit = 50 }) => ({ rows: await readAudit({ limit, env }) }))
