...
import { markPart, pruneErrors, recordError, setForm } from './health.js'
...
export function apply(ctx, config = {}) {
  ...
  // 插件体检：每一块注册的结果都记下来（/vps-doctor、设置页「反馈与诊断」、每天的兼容性检查都看）。
  ...
  registerTools(ctx, deps)
    .then((registered) => ok('tools', `${registered.length} 个`))
  ...
  ctx.inject(['commands'], (cmdCtx) => {
    try {
      const commands = registerCommands(cmdCtx, deps)
      ok('commands', `${commands.count} 条`)
    } catch (error) {
