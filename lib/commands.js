...
export function registerCommands(ctx, deps = {}) {
  ...
  reg({
    ...
    description:
      ...
      '危险命令会先拦下，发 /vps-yes 确认才执行。用法：/vps-sh <命令>',
    input: { hint: '<命令>' },
    async handler(invocation) {
      ...
      try {
        ...
        if (tier === 'danger' && !confirmed) {
          ...
          arm(invocation, { alias, again: `/vps-sh ${raw}`, execute: () => execSh({ alias, raw, tier }) })
          return {
            kind: 'error',
            text: [
        ...
        return await execSh({ alias, raw, tier })
      } catch (error) {
        return { kind: 'error', text: error.message }
      }
    },
  }, { group: '看信息', usage: '/vps-sh <命令>', short: '在当前机器上直接执行一条命令，输出进对话' })

  async function execSh({ alias, raw, tier }) {
    ...
    // 折叠后只看得到第一行：短输出直接跟在命令后面
    const firstOut = body.split('\n')[0]?.trim() ?? ''
    const head = `[${alias}] $ ${raw}${firstOut ? `　${firstOut.slice(0, 80)}` : ''}`
    const tail = []
    if (res.status === 'detached') tail.push(`（还在跑，任务 ${res.taskId}，用 /vps-task ${res.taskId} 查看）`)
