...
export function registerCommands(ctx, deps = {}) {
  ...
  reg({
    name: 'vps-reboot',
    description:
      '重启服务器。两段式：先检查为什么该重启、现在能不能重启、会停哪些容器；加 --yes 才重启，' +
      '并一直等它回来，报告内核、容器和失败的服务。用法：/vps-reboot [-h 别名] [--yes]',
    async handler(invocation) {
      try {
        const args = parseArgs(invocation.rawInput)
        const alias = await resolveAlias(args.host, env, invocation)
        ...
        const run = (body, o = {}) => runRemote({ alias, body, env, runner, ...o })

        if (!args.flags.has('yes')) {
          const out = formatPlan({ alias, head, check: await rebootCheck({ run }) })
  ...
  }, { group: '机器', usage: '/vps-reboot [--yes]', short: '重启并等它回来：先检查，加 --yes 才重启' })
