...
async function resolveAlias(host, env, invocation) {
  ...
  throw new Error(
    Object.keys(doc.hosts).length
      ? [
          '这个对话没有打开 VPS 开关，所以没有默认机器。三选一：',
          '  · 点对话头部的「VPS」开关，选一台（之后这个对话的命令都打它）',
          `  · 这条命令临时加 -h <别名>，例如 -h ${Object.keys(doc.hosts)[0]}`,
          `  · /vps-use <别名> 也能绑定本对话（已登记：${Object.keys(doc.hosts).join('、')}）`,
        ].join('\n')
...
export function registerCommands(ctx, deps = {}) {
  ...
  reg({
    ...
    async handler() {
      ...
      lines.push(
        '',
        '指定机器',
        '  所有命令默认操作当前机器；加 -h <别名> 指定别的，例如 /vps-disk -h jp',
        '  /vps-use <别名> 换当前机器；AI 操作则必须显式说清是哪台',
        '',
        '跟 AI 说话（另外五个工具，直接用自然语言）',
        '  「看看 X 的磁盘」            只读，自动执行，不打扰你',
        '  「给 X 装个 nginx」          有现成菜谱就用菜谱，没有就现写脚本',
        '  「把 a.com 反代到 3000」     改配置文件会自动备份，校验不过自动还原',
        '  「刚才这套存成菜谱」          下次在应用商店里一键重来',
        `  当前确认档位：${doc.settings.confirm}（只读自动 / 改动问你 / 高危标红再问）`,
        '',
        '面板（左边栏「VPS 管理」）',
        '  机器 · 应用商店（装软件）· 系统维护（更新/清理/时区/swap）· 任务；添加机器的向导也在那里',
        '',
      ...
      try {
        ...
        if (tier === 'danger' && !confirmed) {
          ...
          return {
            kind: 'error',
            text: [
              `[${alias}] ⚠ 这条命令被判为高危，没有执行`,
              info.address,
              '',
              `原因：${why}`,
              '',
              '确认要执行就重发一次，把 --yes 放在最前面：',
              `/vps-sh --yes ${raw}`,
            ].join('\n'),
          }
        }
  ...
  }, { group: '机器', usage: '/vps-list', short: '已登记的机器和状态（★ 是当前机器）' })
    ...
    async handler(invocation) {
      ...
      if (!arg) {
        ...
        return {
          kind: 'error',
          text: [
            bound ? `这个对话当前绑定：${bound}` : '这个对话没有绑定机器',
            `用法：/vps-use <别名>　关闭：/vps-use off`,
            `已登记：${Object.keys(doc.hosts).join('、') || '无'}`,
          ].join('\n'),
        }
      }
      if (!sessionId) {
      ...
      if (arg === 'off' || arg === 'none') {
        ...
        return { kind: 'success', text: '已关闭这个对话的 VPS 模式。之后的 /vps-* 命令需要 -h 指定机器；AI 也不会再默认操作任何机器。' }
      }
      if (!doc.hosts[arg]) {
      ...
      return {
        kind: 'success',
        text: `这个对话已绑定 ${arg}：命令不用再写 -h，跟 AI 说话也默认操作它。关闭用 /vps-use off。\n（只影响这个对话，别的对话不受影响）`,
      }
    },
  }, { group: '机器', usage: '/vps-use <别名>', short: '切换当前机器' })
  ...
  }, { group: '安装与任务', usage: '/vps-install <菜谱id> [-h 别名] [--yes]', short: '先出计划，确认后加 --yes 才执行' })
  ...
  }, { group: '安装与任务', usage: '/vps-tasks [任务号] [-h 别名]', short: '远端任务与日志（断线后仍在跑的那些）' })
