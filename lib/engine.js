...
import { SshError, classifySshFailure, describeTarget, sshResolve, sshRun } from './ssh.js'
...
/** 给人和模型看的一行抬头：[hk · 1.2.3.4:22 · 生产] */
export async function targetLabel(alias, group = '', options = {}) {
  try {
    const resolved = await sshResolve(alias, options)
    const g = group ? ` · ${group}` : ''
    return `[${describeTarget(alias, resolved)}${g}]`
  } catch {
    return `[${alias}]`
  }
}
