...
/** 计划页（不带 --yes）：第一行就是结论。返回 { kind, text } */
export function formatPlan({ alias, head, check }) {
  if (!check.ok) return formatResult({ alias, head, result: { phase: 'check', res: check.res } })
  const { kv, assessment: a } = check
  const confirm = `/vps-reboot -h ${alias} --yes`
  const lines = []
  ...
  if (!a.blockers.length) {
    lines.push('', '重启期间机器连不上，一般一两分钟。发 --yes 后会一直等它回来，再报告内核、容器和失败的服务')
  }
