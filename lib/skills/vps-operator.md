...
12. **重启服务器请让用户发 `/vps-reboot`**，不要自己用 `vps_exec` 跑 `reboot`：它会先检查包管理器是不是正在装东西、有没有任务在跑、哪些容器不会自己起来，重启后还会等机器回来并报告。你自己发 `reboot` 的话，连接一断结果会显示成 `detached`（「还在跑」），这是错的，也没人等它回来
