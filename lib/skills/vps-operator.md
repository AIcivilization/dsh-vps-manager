...
13. **要一步步按键的操作交给用户在终端里做。** `vps_exec` 没有交互终端，会等输入的程序（菜单脚本、交互式安装向导、`vim`、`top`、`mysql` 命令行）要么卡到超时，要么直接失败。能写成一条非交互命令的就写成命令；写不了的，告诉用户点对话头部方块后面的 `>_` 打开终端自己操作，并说清楚要选哪一项、输入什么
