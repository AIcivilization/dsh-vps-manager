...
const PRIV_LABEL = { root: 'root', sudo: '免密 sudo', readonly: '只读（不是 root，也没有免密 sudo）' }
...
export async function boundText(alias, env = process.env) {
  ...
  if (f.os_id) {
    ...
    if (f.privilege === 'readonly') lines.push('这台机器没有管理员权限：会改系统的操作做不了，先告诉用户。')
  } else {
...
async function userMessage(text) {
  ...
  const source = { kind: 'plugin', plugin: PLUGIN, form: 'snapshot', sections: [{ name: 'vps-mode', text }] }
  try {
...
export function registerVpsMode(ctx, deps = {}) {
  ...
  return ctx.on('agent/pre-step', async (payload, next) => {
    ...
    try {
      ...
      const text = plan === 'off' ? UNBOUND_TEXT : await boundText(plan, env)
      announced.set(sessionId, plan)
      return { ...decision, messages: [...(decision.messages ?? []), await userMessage(text)] }
    } catch (error) {
