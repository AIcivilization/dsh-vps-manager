...
export async function recipeAction({
  ...
  if (action === 'verify') {
    ...
    const info = await hostContext(alias, { env, sshOptions })
    if (!recipe.verify) {
      return { ok: true, host: alias, status: STATUS.done, output: '', hint: '这条菜谱没有写 verify，只能说执行完了，说不了装好了' }
    }
...
/**
 * 打开 VPS 开关时调用：这台机器还没体检过，就在后台体检一次，不等结果。
...
export function probeIfUnknown(alias, { env = process.env, runner } = {}) {
  import('./config.js')
    ...
    .then((state) => (state.hosts?.[alias]?.facts?.os_id ? null : probeHost({ alias, env, runner })))
