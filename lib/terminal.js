...
export function adaptInteractive(raw) {
  ...
  if (EDITORS.has(name)) {
    return {
      refuse: `${name} 是交互式编辑器，对话里没法用。看内容：/vps-sh cat ${args || '<文件>'}；要修改就跟 AI 说，它会用 vps_write_file 改（自动备份，改坏自动还原）`,
    }
  }
  if (REPL_EXAMPLES[name] && args === '' && !after) {
    return { refuse: `${name} 会进入交互界面，对话里没法用。把要做的事写成一条命令，比如 ${REPL_EXAMPLES[name]}` }
  }
  ...
  if (name === 'docker' && /^exec\b/.test(args) && /(^|\s)-(it|ti|t|i)(\s|$)/.test(args) && /(^|\s)(bash|sh|ash|zsh)$/.test(args)) {
    return { refuse: '进容器的交互 shell 对话里没法用。把命令直接写在后面，比如 /vps-sh docker exec <容器> ls /' }
  }
