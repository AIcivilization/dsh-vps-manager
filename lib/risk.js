...
const DANGER_RULES = [
  ...
  { category: 'disk', why: '磁盘 / 分区操作', re: /\bmkfs(\.\w+)?\b|\bdd\b[^\n]*\bof=|\bfdisk\b|\bparted\b|\bwipefs\b|\b(lv|vg|pv)remove\b/ },
  {
    ...
    why: '改防火墙，可能把自己锁在门外',
    re: /\b(ufw|iptables|ip6tables|nft|firewall-cmd)\b(?![^\n;|&]*(status|-L\b|-S\b|\blist\b|--list))/,
  },
  ...
  { category: 'power', why: '重启或关机', re: /\breboot\b|\bshutdown\b|\bpoweroff\b|\bhalt\b|\binit\s+[06]\b/ },
...
export function classifyScript(script) {
  ...
  for (const rule of DANGER_RULES) {
    ...
    if (m) dangers.push({ category: rule.category, why: rule.why, match: m[0].trim().slice(0, 80) })
  }
